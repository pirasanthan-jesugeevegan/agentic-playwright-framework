...
- **AI failure diagnosis** (`explain-failures.mjs`) - groups any failed/broken Allure results by suite (`group-failures.mjs`), builds a prompt per group (`build-prompt.mjs`) embedding `docs/STATUS.md` for drift context and the git diff of the push that triggered the run (bounded to 12,000 characters, so a failure can be tied to the line that changed - or the model can say the diff doesn't explain it, which points at the app instead), and asks Claude for a root cause, cross-suite correlation, and STATUS.md drift check. Writes `ai-diagnosis.json` alongside the Allure report - a safe no-op (empty object) on a clean run.
